...
int edge_init(Keystone::Enclave* enclave) {
  ...
  register_call(OCALL_INIT_SERV_CONN, init_connection_serv_wrapper);
  register_call(OCALL_WAIT_FOR_CONN, wait_for_conn_serv_wrapper);
  register_call(OCALL_SEND_FD, send_message_fd_wrapper);
  register_call(OCALL_RECV_FD, recv_message_fd_wrapper);
  register_call(OCALL_GET_USER_RECORD, get_user_record_wrapper);
  register_call(OCALL_SET_USER_RECORD, set_user_record_wrapper);
  register_call(OCALL_GET_RULE_RECORD, get_rule_record_wrapper);
  register_call(OCALL_SET_RULE_RECORD, set_rule_record_wrapper);
...
void init_connection_serv_wrapper(void *buffer) {
  ...
  int32_t fd = initiate_connection_serv((char *)hostname, portnumber);
...
void wait_for_conn_serv_wrapper(void *buffer) {
  ...
  int32_t servfd = *((int32_t *)call_args);

  int clientfd = accept_connection(servfd);

  uintptr_t data_section = edge_call_data_ptr();

  memcpy((void*)data_section, &clientfd, sizeof(int32_t));
...
void get_user_record_wrapper(void *buffer) {
  ...
  get_user_record_request_t *req = (get_user_record_request_t *) call_args;
  
  struct enc_keystore_user user;
  int32_t exists = get_user_record(req->username, &user);

  if (req->reply) {
    if (edge_call_setup_wrapped_ret(edge_call, &user, (exists == 1) ? sizeof(struct enc_keystore_user) : 0)) {
      edge_call->return_data.call_status = CALL_STATUS_BAD_PTR;
    }
    else {
      edge_call->return_data.call_status = CALL_STATUS_OK;
    ...
    memcpy((void*)data_section, &exists, sizeof(int32_t));
...
void set_user_record_wrapper(void *buffer) {
  ...
  set_user_record_request_t *req = (set_user_record_request_t *) call_args;
  
  int32_t error = set_user_record(req->username, &(req->enc_user));
...
void get_rule_record_wrapper(void *buffer) {
  ...
  get_rule_request_t *req = (get_rule_request_t *) call_args;
  
  struct enc_keystore_rule rule;
  int32_t exists = get_rule_record(req->uid, req->rule_id, &rule, (req->reply == 1) ? 0 : 1);

  if (req->reply) {
    if (edge_call_setup_wrapped_ret(edge_call, &rule, (exists == 1) ? sizeof(struct enc_keystore_rule) : 0)) {
      edge_call->return_data.call_status = CALL_STATUS_BAD_PTR;
    }
    else {
      edge_call->return_data.call_status = CALL_STATUS_OK;
...
void set_rule_record_wrapper(void *buffer) {
  ...
  set_rule_request_t *req = (set_rule_request_t *) call_args;
  
  int32_t error = set_rule_record(req->uid, req->rule_id, &(req->enc_rule));
